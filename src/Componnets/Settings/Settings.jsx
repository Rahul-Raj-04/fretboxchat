...
function Settings() {
  ...
  const updateUser = async (data) => {
    ...
    try {
      ...
      const responseText = await response.text(); // ✅ Response ko text format me log karna
  ...
  const handleProfilePictureChange = async (event) => {
    ...
    try {
      const response = await fetch(
        `${Baseurl}/api/v1/user/profilephoto?userId=${userId}`,
        {
          method: "POST",
          body: formData,
        }
      ...
      if (response.ok) {
        // Refresh to update UI with new profile pic
      } else {
      ...
      <div className="tab-content active w-full md:w-[450px] lg:w-[380px] max-w-full shadow  h-screen overflow-y-hidden mb-[80px] lg:mb-0 bg-red-50 border-x-[1px] border-gray-200">
        <div>
          ...
          <div className="p-6 h-[650px] overflow-y-auto custom-scrollbar">
            ...
            />
            <Privacy userdetails={user} loading={loading} />
          </div>
        </div>
      </div>
    </>
