...
function Groupchatfooter() {
  ...
  const { sendMessage, sendMessageWithPoll, selectedChat } = useChatStore();
  ...
  const handleSendMessage = async (e) => {
    ...
    if (fileInputRef.current?.files[0]) {
      formData.append("image", fileInputRef.current.files[0]); // Send file instead of Base64
    }

    try {
      await sendMessage(formData, selectedChat._id); // Send FormData instead of JSON
      setText("");
      setImagePreview(null);
      if (fileInputRef.current) fileInputRef.current.value = "";
